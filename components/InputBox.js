...
function InputBox() {
  ...
  const sendPost = async (e) => {
    ...
    await uploadString(storageRef, imageToPost, "data_url").then(
      async (snapshot) => {
    ...
    //clears out the image
    removeImage();
    //clears out the input field after using it
    ...
    <div className="bg-white p-2 rounded-2xl shadow-md text-gray-500 font-medium mt-6">
      <div className="flex items-center space-x-4 p-4">
        ...
        <form className="flex flex-1">
          <input
            ref={inputRef}
            className="rounded-full outline-none h-12 bg-gray-100 flex-grow px-5"
            type="text"
